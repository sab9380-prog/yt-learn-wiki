...
## 📚 핵심 용어
- **Missions:** 여러 AI 에이전트가 역할을 분담해 며칠간 자율적으로 소프트웨어 개발을 수행하는 시스템 / 마치 각자 전문 분야가 다른 팀원들이 프로젝트 매니저 없이도 며칠간 협업하는 것과 같다 / 단일 AI 에이전트는 혼자 모든 일을 하려는 1인 개발자라면, Missions는 기획자-개발자-QA가 분업하는 팀
- **Validation Contract:** 코드 구현 전에 미리 정의하는 수백 개의 검증 기준으로, 완료 조건을 구현과 독립적으로 명시 / 요리하기 전에 미리 정한 맛 기준표 - 요리사가 바뀌어도 같은 기준으로 평가 / 구현 후 작성하는 테스트는 만든 음식에 맞춰 기준을 조정하는 것, Validation Contract는 미리 정한 절대 기준
- **구조화된 핸드오프:** 에이전트 간 작업 전달 시 완료사항, 미완료사항, 실행 명령어, 발견된 이슈를 체계적으로 문서화하는 방식 / 교대 근무 시 인수인계서를 자세히 작성하는 것과 같다 / 일반적인 에이전트 통신은 '끝났다'고 말하는 것, 구조화된 핸드오프는 상세한 업무일지 전달
- **Creator-Verifier 패턴:** 하나의 에이전트가 작업을 수행하면 별개의 에이전트가 검증하는 분리된 역할 구조 / 글쓴이와 교정자가 다른 사람인 것처럼, 만든 사람과 검토하는 사람을 분리 / Self-validation은 자기가 만든 것을 자기가 검토하는 것, Creator-Verifier는 제3자 관점 검토
