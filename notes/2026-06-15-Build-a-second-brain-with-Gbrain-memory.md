...
## 📚 핵심 용어
- **G brain:** 노트와 콘텐츠를 자동으로 수집하여 검색 가능한 지식 그래프로 변환하는 오픈소스 도구 / 디지털 도서관의 사서 같은 존재로, 흩어진 정보를 자동으로 분류하고 연결해주는 시스템 / ChatGPT 메모리 vs G brain — 하나는 대화 기록 저장, 하나는 구조화된 지식 네트워크 구축
- **하이브리드 검색:** 키워드 검색, 의미 검색, 관계 검색을 동시에 사용하는 다층적 검색 방식 / 여러 개의 렌즈를 겹쳐 쓰는 것처럼, 다양한 방식으로 동시에 정보를 찾는 방법 / 단일 검색 vs 하이브리드 검색 — 하나는 한 가지 방식으로만, 하나는 여러 방식을 조합해 더 정확한 결과
- **자동 연결 (Self-wiring):** 새로운 노트가 추가될 때 사람, 회사, 전략 등을 자동으로 기존 데이터와 연결하는 기능 / 새로운 사람을 만날 때 자동으로 공통 지인을 찾아주는 스마트한 인맥 관리 시스템 / 수동 태깅 vs 자동 연결 — 하나는 사용자가 직접 분류, 하나는 시스템이 자동으로 관계를 발견
- **백링크 부스팅:** 검색 결과 순위를 매길 때 텍스트 유사도뿐만 아니라 연결 관계의 밀도도 고려하는 방식 / 학술 논문의 인용 횟수처럼, 많이 연결된 정보일수록 더 중요하다고 판단하는 방식 / 단순 키워드 매칭 vs 백링크 부스팅 — 하나는 단어 일치만, 하나는 정보의 중요도와 연결성까지 고려
