...
## 📚 핵심 용어
- **LLM 위키:** 안드레 카파시가 제안한 지식 관리 방법론으로, AI를 활용해 수집한 정보를 위키 형태로 체계화하고 연결하는 시스템 / 전통적인 위키피디아처럼 정보를 연결하되, AI가 편집자 역할을 해서 자동으로 관련성을 찾고 분류하는 개인 백과사전 / RAG vs LLM 위키 — RAG는 벡터 DB로 검색하는 도서관, LLM 위키는 AI가 연결고리를 만들어주는 살아있는 지식 네트워크
- **목적성 있는 수집:** 단순히 정보를 모으는 것이 아니라 '왜 이것을 수집하는지' 명확한 이유와 목적을 가지고 지식을 축적하는 방식 / 무작정 물건을 사 모으는 것이 아니라, 특정 프로젝트나 목표를 위해 필요한 재료만 골라서 수집하는 것 / 목적성 수집 vs 무작정 수집 — 전자는 골드 데이터가 되고, 후자는 쓰레기 데이터가 되어 GIGO(Garbage In Garbage Out)를 초래
- **골드 인 골드 아웃:** 고품질의 목적 있는 데이터를 입력하면 고품질의 결과물이 나온다는 원칙 (기존 GIGO의 반대 개념) / 요리할 때 좋은 재료를 쓰면 맛있는 음식이 나오는 것처럼, AI에게 의미있는 데이터를 주면 가치있는 결과가 나온다 / GIGO vs GIGO — 쓰레기를 넣으면 쓰레기가 나오지만, 금을 넣으면 금이 나온다
- **인젝스트 프로세스:** 수집한 원시 정보를 AI가 분석하여 개념과 개체를 분리하고 기존 지식과 연결해 위키에 정리하는 과정 / 날것의 식재료를 세척, 손질, 조리해서 먹을 수 있는 음식으로 만드는 과정 / 단순 저장 vs 인젝스트 — 전자는 그냥 쌓아두는 것, 후자는 소화 가능한 지식으로 변환하는 것
